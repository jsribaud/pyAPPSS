    ...
    for agc in agcs:
        ...
        try:
            ...
            if '.fits' in agc:
                ...
                filename = f"{agc}_spectra.fits"
                # assume it's an ALFALFA number. User should input "ALFALFA_###" to specify that it is a non AGC ###.

            else:
                filename = f"AGC{agc}.fits"
                # assume it's an AGC number
            ...
            if not mgauss:
                b = baseline.Baseline(filename, smooth_int=smo, path=path, noconfirm=noconfirm, dark=dark)
                vel, spec, rms = b()
                measure.Measure(smo=smo, gauss=gauss, twopeak=twopeak, trap=trap, path=path, dark=dark,
            ...
            if mgauss:
                ...
                print("You have selected to use the multigauss fit. Please note this will automatically smooth the data"
                      "with a boxcar of 21.\n")
                b = baseline.Baseline(agc, smooth_int=smo, path=path, noconfirm=noconfirm, dark=dark)
                vel, spec, rms = b()
