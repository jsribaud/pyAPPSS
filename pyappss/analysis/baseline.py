...
class Baseline:
    ...
    def __init__(self, filename, smooth_int, path="", noconfirm=False, dark=False):
        ...
        elif 'ALFALFA' in filename:
            self.filename = '{}_spectra.fits'.format(filename)
        else:
    ...
    def __plot(self, xmin=None, xmax=None, ymin=None, ymax=None):
        ...
        #create a title for the plot by truncating some text
        if 'ALFALFA' in self.filename:
            title = self.filename[0:-13]    #truncates "_spectra.fits" for ALFALFA sample
        else:
            title = self.filename[0:-5]     #truncates ".fits" for AGC sample
