...
class ManyGauss:
        ...
        def gauss_edges(self, manualpeaks=False):
                ...
                if peakstruth:
                        ...
                        leftmax = np.where(self.gaussian_model == max(self.gaussian_model[left-5, leftchan+5]))
                ...
                for i in range(len(self.x)):  # iterating over the shortened spectrum so we have consistency in the indices
                        if self.gaussian_model[i] >= percent15_l and self.gaussian_model[i] <= percent85_l and self.x[i] >= self.x[self.leftedge_chan] and self.x[i] <= self.x[leftmax]: #LEFT
                        ...
                        if self.gaussian_model[i] >= percent15_r and self.gaussian_model[i] <= percent85_r and self.x[i] >= self.x[rightmax] and self.x[i] <= self.x[self.rightedge_chan]: #RIGHT
                ...
                try:
                        ...
                        for i in range(len(self.x)):  # iterating over the shortened spectrum so we have consistency in the indices
                                if self.gaussian_model[i] >= percent15_l and self.gaussian_model[i] <= percent85_l and self.x[i] >= self.vel1 and self.x[i] <= self.x[leftmax]: #LEFT
                                        region_l.append(i)  # appending the indices where the y values are between 15 and 85%, and where the x values are within the selected region.
                                        #Wrapped into one for loop since I define stuff like this. It'd be six of one half a dozen of another to do it the way twopeak calc/edgefit does, where it does if left and then if right
                                if self.gaussian_model[i] >= percent15_r and self.gaussian_model[i] <= percent85_r and self.x[i] >= self.x[rightmax] and self.x[i] <= self.vel2: #RIGHT
                                        region_r.append(i)  # appending the indices where the y values are between 15 and 85%, and where the x values are within the selected region.
                        p15chan_l, p15chan_r = min(region_l), min(region_r)
                        p85chan_l, p85chan_r = max(region_l), max(region_r)
                        print('Determining edges using gaussians did not work - defaulting to determining edges with original region')
